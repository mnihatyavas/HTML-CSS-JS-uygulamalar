(()=> {
    const rasgele16Baytlarý = [180,253,62,216,47,35,90,55,218,233,103,10,172,143,161,177];
    const rasgele12Baytlarý = [212,187,26,247,172,51,37,151,27,177,249,142];
    const sarýlýAnahtarBaytlarý = [6,155,182,208,7,141,44,18,3,151,58,126,68,100,252,
    ...
    let sarýmsýzlananÝmzalamaAnahtarý;
    const dijitalimzaDüðmesi = document.querySelector (".pkcs8 .imzala-düðmesi");
    ...
    async function sarýmsýzlamaAnahtarýnýAl() {
        const anahtarMalzemesi = await anahtarMalzemesiniAl();
        const rasgele16Tamponu = baytdanDiziTamponuna (rasgele16Baytlarý);
    ...
    async function özelAnahtarýSarýmsýzla (sarýmlýAnahtar) {
        const sarýmsýzlayanAnahtar = await sarýmsýzlamaAnahtarýnýAl();
        const sarýmlýAnahtarDiziTamponu = baytdanDiziTamponuna (sarýmlýAnahtar);
        const rasgele12Tamponu = baytdanDiziTamponuna (rasgele12Baytlarý);
        return window.crypto.subtle.unwrapKey (
            "pkcs8",
            sarýmlýAnahtarDiziTamponu,
            sarýmsýzlayanAnahtar,
    ...
    async function mesajýDijitalmzala() {
        const kodlananMesaj = mesajKodlayýcýsýnýAl();
        const dijitalÝmza = await window.crypto.subtle.sign (
            {name: "RSA-PSS", saltLength: 32,},
            sarýmsýzlananÝmzalamaAnahtarý,
            kodlananMesaj
        ); // con..sonu...
        const dijitalimzaDeðeri = document.querySelector (".pkcs8 .dijitalimza-deðeri");
        dijitalimzaDeðeri.classList.add ("netleþ");
        dijitalimzaDeðeri.addEventListener ("animationend", ()=> {dijitalimzaDeðeri.classList.remove ("netleþ");});
        const tampon = new Uint8Array (dijitalÝmza, 0, 5);
        dijitalimzaDeðeri.textContent = `${tampon}...[Toplam ${dijitalÝmza.byteLength} byte]`;
    } // asy..sonu...

    function imzaDüðmesiniSýfýrla() {
        dijitalimzaDüðmesi.setAttribute ("disabled", true);
        dijitalimzaDüðmesi.classList.add ("gizli");
    } // func sonu...

    function imzalaDüðmesiniEtkinleþtir() {
        dijitalimzaDüðmesi.classList.add ("netleþ");
        dijitalimzaDüðmesi.addEventListener ("animationend", ()=> {dijitalimzaDüðmesi.classList.remove ("netleþ");});
        dijitalimzaDüðmesi.removeAttribute ("disabled");
        dijitalimzaDüðmesi.classList.remove ("gizli");
    } // func sonu...

    const sarýmsýzlamaAnahtarýDüðmesi = document.querySelector (".pkcs8 .anahtarý-sarýmsýzla-düðmesi");
    sarýmsýzlamaAnahtarýDüðmesi.addEventListener ("click", async ()=> {
        try {
            sarýmsýzlananÝmzalamaAnahtarý = await özelAnahtarýSarýmsýzla (sarýlýAnahtarBaytlarý);
            imzalaDüðmesiniEtkinleþtir();
        }catch (olay) {
            imzaDüðmesiniSýfýrla();
            alert("Yanlýþ þifre girdiniz");
        } // try-catch sonu...
    }); // sar..sonu...
    dijitalimzaDüðmesi.addEventListener ("click", mesajýDijitalmzala);
})();